...
class Owner {
    ...
    func returnBook(book: Book, owner: Owner) {
        if borrowedBooks.contains(where: { $0.bookID == book.bookID }) {
            owner.borrowedBooks.removeAll { $0.bookID == book.bookID }
            book.isBorrowed = false
        } else {
...
person2.borrowBook(book: andAnother, owner: person2)
...
class Cart {
    ...
    func removeProduct(product: Product) {
        items.removeAll { $0.productID == product.productID }
    }
...
class User {
    ...
    var user1 = User(userID: 11, username: "number 1 always", cart: cart1)
    var user2 = User(userID: 12, username: "Silver medalist", cart: cart2)

user1.addToCart(item: product1)
user1.addToCart(item: product1)
user2.addToCart(item: product5)
user1.addToCart(item: product4)
user2.addToCart(item: product3)
user2.addToCart(item: product2)
user2.addToCart(item: product4)

cart1.fullPrice()
cart2.fullPrice()

user1.removeFromCart(item: product4)
